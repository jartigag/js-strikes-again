...
___ Use the reduce Method to Analyze Data ___
...
function getRating(watchList){
  ...
  //var averageRating = watchList.reduce((obj, movie) => {obj = {"director": movie.Director, "rating": movie.imdbRating}; return obj}, {});

  const averageRating = watchList.reduce(
    (
      { sum, count },                        // accumulator (it gets assigned the return value of the callback function from the previous iteration)
      { Director: dir, imdbRating: rating }, // current element
      idx,                                   // current index
      arr                                    // array upon which reduce is called
    ) => {
      if (dir==='Christopher Nolan') { count++; sum +=Number(rating); }
      if (idx===arr.length-1)
        return sum/count;
      else
        return {sum, count};
    }, { sum: 0, count: 0 }                  // initial value for the accumulator
  );
